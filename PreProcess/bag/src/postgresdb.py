...
class Database:
    ...
    def selectQuery(self, sql, parameters=None):
        try:
            if parameters:
                self.cursor.execute(sql, parameters)
            else:
    ...
    def insertMultiple(self, table, tupleList):
        try:
            self.verbind()
            sql = self.cursor.mogrify("INSERT INTO " + table + " VALUES " + ','.join(["%s"] * len(tupleList)), tupleList)
            self.cursor.execute(sql)
            self.commit()
        except Exception as e:
            Log.log.error("fout %s voor query: %s" % (str(e), str(sql)))
            self.log_actie("insertMultiple", "n.v.t", "fout=%s" % str(e), True)
            raise
